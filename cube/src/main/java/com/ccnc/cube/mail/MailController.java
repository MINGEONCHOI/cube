...
//====================================================================================	       보낸 메일함(완)  
	...
	public String sendMailList(Model model, HttpSession session) {
		System.out.println("보낸 메일 리스트 요청됨");
		Users users = (Users)session.getAttribute("login_user");
		String usersMail = users.getUserEmail();
		model.addAttribute("sendMailList",mailService.sentMailbox(usersMail, MailIsDelete.존재, MailImportant.기본));	
		model.addAttribute("asidePage", "./mail/mail_aside.jsp");
...
//====================================================================================	       받은 메일함(완)
	...
	public String receiveMailList(Model model, HttpSession session) {
	    System.out.println("받은 메일 리스트 요청됨");
	    Users users = (Users) session.getAttribute("login_user");
	    String usersEmail = users.getUserEmail();

	    // 받은 메일 데이터를 가져온다.
	    List<ReceiveMail> receivedMailList = mailService.receivedMailBox(usersEmail, MailIsDelete.존재, MailImportant.기본);

	    // 예약된 메일을 현재 시간과 비교하고 필터링한다.
	    List<ReceiveMail> filteredMailList = receivedMailList.stream()
	            .filter(mail -> mail.getReceiveMailReservationDate() == null || LocalDateTime.now()
	                    .isAfter(mail.getReceiveMailReservationDate()) || LocalDateTime.now()
	                    .isEqual(mail.getReceiveMailReservationDate()))
	            .collect(Collectors.toList());

	    model.addAttribute("receivedMailList", filteredMailList);
	    model.addAttribute("asidePage", "./mail/mail_aside.jsp");
...
//====================================================================================	       삭제 메일함(완)
	...
	@GetMapping("/isdeletemail_list")
	public String sendIsdeleteMailList(Model model,HttpSession session) {
...
//====================================================================================	       메일 보내기 기능(완)
	  ...
	  public ResponseDTO<?> mailSend(@RequestBody SendMail sendMail) {
	  System.out.println("메일보내기 요청");
	  mailService.saveSendMail(sendMail);
	  String[] receiverArray = sendMail.getSendMailReceiverEmail().split(";");
	  for (String receiver : receiverArray) {
		  ReceiveMail receiveMail = new ReceiveMail(sendMail);
		  receiveMail.setReceiveMailReceiverEmail(receiver);
