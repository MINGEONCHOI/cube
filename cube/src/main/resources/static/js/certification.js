let certification = {
	...
	checkMobile: function() {
		...
		$.ajax({
			...
			alert(message);
			if(response.status === 200) {
				document.getElementById("mobileStatus").innerHTML = "사용 가능한 휴대전화번호";
                document.getElementById("mobileStatus").style.color = "blue";			
			} else {
				document.getElementById("mobileStatus").innerHTML = "사용 불가능한 휴대전화번호";
	...
	signUp: function() {
		...
		if (idStatusVal.trim() !== "사용 가능한 아이디") {
		    ...
		    alert("아이디 중복확인이 필요합니다. (5자 이상 입력)");
		    return;
		}
		//비밀번호 검사
		...
		if(!user.userGender) {
			alert("성별을 선택해주세요.");
			document.getElementById("userGenderError").innerHTML = "성별을 선택해주세요.";
		...
		//조건 확인
		if (mobileStatusVal.trim() !== "사용 가능한 휴대전화번호") {
